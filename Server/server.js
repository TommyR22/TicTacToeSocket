  ...
  socket.on('disconnect', function(){
    ...
    if (room) {
      ...
      if(io.sockets.adapter.rooms[room]) {  // if UNDEFINED -> stanza eliminata
        ...
        socket.to(room).emit('notifyPlayerDisconnected', { oldClient: client_list[socket.id], clients: clientsInRoom });
      }
    }
...
//     key1 in map;
  ...
  socket.on('send.message', function(data){
    console.log('sendMessage', data.room);
    // send message to all client in the room
    socket.to(data.room).emit('send.message.from', data.msg);
  });
