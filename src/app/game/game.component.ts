...
export class GameComponent implements OnInit {
    ...
    try {
      ...
      this.socket.on('send.message.from', (data) => {
        ...
        if (data.rematch === 'enable') {
          ...
          // TODO emit new event reset table-component
        }
              ...
              if (now >= end) {
                ...
                for (let i = 0; i < this.matrix.length; i++) {
                  if (this.matrix[i] === 0) {
                    ...
                    this.progressWidth = 0;
                    clearInterval(this.progressInterval);
                    this.socketService.send(this.socket, this.nameRoom, { matrix: this.matrix, userCurrentTurn: this.userCurrentTurn, winner: 'tie', rematch: '' });
                  }
                }
              }
            }, 10);
  ...
  cubeSelected(cube: number) {
    ...
    if (result) {
      this.socketService.send(this.socket, this.nameRoom, { matrix: this.matrix, userCurrentTurn: this.userCurrentTurn, winner: this.user.id, rematch: ''  });
    } else {
      this.socketService.send(this.socket, this.nameRoom, { matrix: this.matrix, userCurrentTurn: this.userCurrentTurn, winner: '', rematch: ''  });
    }
  }
  ...
  rematch() {
    this.showResult = false;
    this.socketService.send(this.socket, this.nameRoom, { matrix: this.matrix, userCurrentTurn: this.userCurrentTurn, winner: '', rematch: 'enable' });
  }
